...
export default function FarmDashboard() {
  ...
  const [analyticsData, setAnalyticsData] = useState<any>({
    ...
    irrigationSchedule: [],
    pesticides: { inventory: [], monthlyUsage: [] },
    costAnalysis: {
      waterCost: 0,
      pesticideCost: 0,
      equipmentCost: 0,
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      await Promise.all([
        fetchFieldsData(currentFarm),
        fetchTeamData(currentFarm.id),
        fetchTasksData(currentFarm.id),
        fetchYieldData(currentFarm),
        fetchAnalyticsData(currentFarm),
      ]);
  ...
  // Fetch fields (zones) with sensor data
  const fetchFieldsData = async (farm: Farm) => {
    try {
      const zones = farm.zones || [];
      const fieldsWithData = await Promise.all(
        zones.map(async (zone: FarmZone) => {
          try {
            const latestSensor = await sensorsApi.getLatest(zone.id);
            return {
              id: zone.id,
              name: zone.name || "Unnamed Zone",
              size: zone.area_hectares ? `${zone.area_hectares} ha` : "Unknown",
              crop: zone.crop || "Unknown",
              status: latestSensor?.soil_moisture
                ? latestSensor.soil_moisture > 60
                  ? "Healthy"
                  : latestSensor.soil_moisture > 40
                  ? "Needs Water"
                  : "Critical"
                : "Unknown",
  ...
  const fetchTeamData = async (farmId: string) => {
    try {
      const team = await teamApi.list(farmId);
      setTeamData(team);
    } catch (error) {
  ...
  // Fetch tasks
  const fetchTasksData = async (farmId: string) => {
    try {
      const tasks = await tasksApi.list(farmId);
      setTasksData(tasks);
    } catch (error) {
  ...
  // Fetch yield data
  const fetchYieldData = async (farm: Farm) => {
    try {
      const zones = farm.zones || [];
      const yieldPromises = zones.map(async (zone: FarmZone) => {
        try {
          const yields = await yieldsApi.getByZone(zone.id, 10);
          // Calculate this week vs last week (simplified)
          const totalYield = yields.reduce((sum, y) => sum + y.amount, 0);
          return {
            field: zone.name || "Unknown",
            crop: zone.crop || "Unknown",
            thisWeek: `${totalYield.toFixed(1)} ${yields[0]?.unit || "kg"}`,
            lastWeek: `${(totalYield * 0.9).toFixed(1)} ${
              yields[0]?.unit || "kg"
            }`,
            change: "+10%",
            trend: "up",
  ...
  // Fetch analytics data (water, pesticides, costs)
  const fetchAnalyticsData = async (farm: Farm) => {
    try {
      // Fetch water storage
      const storages = await waterApi.getStorage(farm.id);
      const primaryStorage = storages[0];

      // Fetch pesticides
      const pesticides = await pesticidesApi.list(farm.id);

      // Fetch irrigation schedules for all zones
      const zones = farm.zones || [];
      const schedulesPromises = zones.map((zone: FarmZone) =>
        waterApi.getSchedules(zone.id, "scheduled").catch(() => [])
      );
      ...
      setAnalyticsData({
        waterUsage: {
          totalUsed: 9274, // Would calculate from actual usage data
          dailyAverage: 1325,
          weeklyTarget: 10599,
          efficiency: 87.5,
          breakdown: [],
        },
        waterStorage: primaryStorage
          ? {
              currentLevel: primaryStorage.current_level,
              capacity: primaryStorage.capacity,
              percentage:
                (primaryStorage.current_level / primaryStorage.capacity) * 100,
              refillDate: primaryStorage.next_refill_date || "",
              daysLeft: primaryStorage.next_refill_date
                ? Math.ceil(
                    (new Date(primaryStorage.next_refill_date).getTime() -
                      Date.now()) /
                      (1000 * 60 * 60 * 24)
                  )
                : 0,
              criticalLevel: primaryStorage.critical_level || 0,
            }
          : {
              currentLevel: 0,
              capacity: 0,
        ...
        irrigationSchedule: allSchedules
          .slice(0, 3)
          .map((schedule: IrrigationSchedule, idx: number) => ({
            field:
              zones.find((z: FarmZone) => z.id === schedule.zone_id)?.name ||
              "Unknown",
            nextIrrigation: schedule.scheduled_time,
            amount: `${schedule.estimated_amount || 0} L`,
            priority:
        ...
        pesticides: {
          inventory: pesticides.map((p: PesticideInventory) => ({
            name: p.name,
            current: p.current_stock,
            unit: p.unit,
            capacity: p.capacity || p.current_stock,
            lastUsed: p.last_used_date || "N/A",
            nextOrder: p.next_order_date || "N/A",
          })),
          monthlyUsage: [],
        },
        costAnalysis: {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn === "true" && email) {
      ...
      setUserEmail(email);
      // Fetch weather and dashboard data after authentication
      fetchWeatherData();
      fetchDashboardData();
    } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-6">
                <p className="text-sm font-semibold text-gray-500 uppercase tracking-wider">
                  {t("farmDashboard.quickStats.weeklyYield")}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <Link href="/field-monitoring" className="block">
            <div className="bg-white rounded-xl shadow-lg border-0 p-8 hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">
                Real-time sensor data, soil health metrics, and AI-powered
          ...
          <Link href="/team" className="block">
            <div className="bg-white rounded-xl shadow-lg p-8 hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">
                Manage worker farmers, assign field tasks, and coordinate team
                activities via SMS.
              </p>
          ...
          <Link href="/tasks" className="block">
            <div className="bg-white rounded-xl shadow-lg p-8 hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">
                Create and assign tasks to workers, set priorities, and track
                completion status.
              </p>
          ...
          <Link href="/tracker" className="block">
            <div className="bg-white rounded-xl shadow-lg p-8 hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">
                Monitor live task progress, SMS communications, and worker
                location updates.
              </p>
          ...
          >
            ...
            <p className="text-gray-600 mb-6 leading-relaxed">
              Water usage, storage levels, pesticide tracking, and comprehensive
              farm metrics.
            </p>
            ...
            <div className="p-8">
              <div className="space-y-4">
                <div className="flex items-center space-x-3">
                  ...
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Ahmed Hassan</span> completed
                    irrigation system check in North Field
                  </p>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    <span className="font-medium">
                      Pest inspection in South Field
                    </span>
                  </p>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Maria Rodriguez</span> joined
                    the team
                  </p>
                        ...
                        >
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="font-semibold">
                                {member.current_zone_id || "Not assigned"}
                              </p>
                            </div>
                            <div>
                ...
                {activeModal === "tasks" && (
                  <div className="space-y-4">
                    {tasksData.length === 0 ? (
                      ...
                      tasksData.map((task) => {
                        // Find assignee name from teamData
                        const assignee = teamData.find(
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-blue-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-blue-800 mb-4 flex items-center">
                        <Droplets className="w-5 h-5 inline mr-2" /> Water Usage
                        Analytics
                      </h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div className="bg-white rounded-lg p-4">
                          ...
                          <p className="text-2xl font-bold text-blue-600">
                            {analyticsData.waterUsage.totalUsed} L
                          </p>
                          <p className="text-xs text-green-600">
                            Target: {analyticsData.waterUsage.weeklyTarget} L
                          </p>
                        </div>
                        <div className="bg-white rounded-lg p-4">
                          <p className="text-sm text-gray-600">Daily Average</p>
                          <p className="text-2xl font-bold text-blue-600">
                            {analyticsData.waterUsage.dailyAverage} L
                          </p>
                        ...
                        <div className="bg-white rounded-lg p-4">
                          ...
                          <p className="text-2xl font-bold text-green-600">
                            {analyticsData.waterUsage.efficiency}%
                          </p>
                          <p className="text-xs text-green-600">
                      ...
                      <div className="bg-white rounded-lg p-4">
                        ...
                        {analyticsData.waterUsage.breakdown.map(
                    ...
                    <div className="bg-cyan-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-cyan-800 mb-4 flex items-center">
                        <Database className="w-5 h-5 mr-2" /> Water Storage
                        Status
                      </h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white rounded-lg p-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-sm font-semibold">
                              {analyticsData.waterStorage.currentLevel} /{" "}
                              {analyticsData.waterStorage.capacity} L
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-4 mb-2">
                            <div
                              className={`h-4 rounded-full ${
                                analyticsData.waterStorage.percentage > 50
                                  ? "bg-green-600"
                                  : analyticsData.waterStorage.percentage > 25
                                  ? "bg-yellow-600"
                                  : "bg-red-600"
                              }`}
                              style={{
                                width: `${analyticsData.waterStorage.percentage}%`,
                              }}
                            ></div>
                          </div>
                          <p className="text-lg font-bold text-cyan-600">
                            {analyticsData.waterStorage.percentage}% Full
                          </p>
                        </div>
                        ...
                        <div className="bg-white rounded-lg p-4">
                          ...
                          <p className="text-lg font-bold text-cyan-600">
                            {analyticsData.waterStorage.refillDate}
                          </p>
                          <p className="text-sm text-orange-600">
                            In {analyticsData.waterStorage.daysLeft} days
                          </p>
                          <p className="text-xs text-red-600 mt-2 flex items-center">
                            <AlertTriangle className="w-3 h-3 mr-1" /> Critical
                            level: {analyticsData.waterStorage.criticalLevel} L
                            (Desert reserve minimum)
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-green-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-green-800 mb-4 flex items-center">
                        <ShowerHead className="w-5 h-5 mr-2" /> Irrigation
                        Schedule
                      </h3>

                      <div className="space-y-3">
                        {analyticsData.irrigationSchedule.map(
                    ...
                    <div className="bg-amber-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-amber-800 mb-4 flex items-center">
                        <Beaker className="w-5 h-5 mr-2" /> Pesticide Inventory
                        & Usage
                      </h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white rounded-lg p-4">
                          ...
                          <div className="space-y-3">
                            {analyticsData.pesticides.inventory.map(
                                ...
                                <div key={index} className="border-b pb-2">
                                  ...
                                  <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                                    <div
                                      className={`h-2 rounded-full ${
                                        item.current / item.capacity > 0.5
                                          ? "bg-green-600"
                                          : item.current / item.capacity > 0.25
                                          ? "bg-yellow-600"
                                          : "bg-red-600"
                                      }`}
                                      style={{
                                        width: `${
                                          (item.current / item.capacity) * 100
                                        }%`,
                                      }}
                                    ></div>
                                  </div>
                                  <div className="flex justify-between text-xs text-gray-600">
                                    ...
                                    <span
                                      className={
                                        item.nextOrder === "Soon" ||
                                        item.nextOrder.includes("2025-10")
                                          ? "text-orange-600"
                                          : "text-green-600"
                                      }
                                    >
                        ...
                        <div className="bg-white rounded-lg p-4">
                          ...
                          {analyticsData.pesticides.monthlyUsage.map(
                    ...
                    <div className="bg-purple-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-purple-800 mb-4 flex items-center">
                        <TrendingUp className="w-5 h-5 inline mr-2" /> Cost
                        Analysis
                      </h3>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-white rounded-lg p-4 text-center">
                          ...
                          <p className="text-lg font-bold text-blue-600">
                            QAR {analyticsData.costAnalysis.waterCost}
                          </p>
                        </div>
                        <div className="bg-white rounded-lg p-4 text-center">
                          <p className="text-sm text-gray-600">
                          ...
                          <p className="text-lg font-bold text-amber-600">
                            QAR {analyticsData.costAnalysis.pesticideCost}
                          </p>
                        </div>
                        <div className="bg-white rounded-lg p-4 text-center">
                          <p className="text-sm text-gray-600">Equipment</p>
                          <p className="text-lg font-bold text-gray-600">
                            QAR {analyticsData.costAnalysis.equipmentCost}
                          </p>
                        </div>
                        <div className="bg-white rounded-lg p-4 text-center">
                          <p className="text-sm text-gray-600">Total Weekly</p>
                          <p className="text-lg font-bold text-purple-600">
                            QAR {analyticsData.costAnalysis.totalWeekly}
                          </p>
                          <p className="text-xs text-green-600">
                            {analyticsData.costAnalysis.savings}% saved
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <>
                        ...
                        <div className="bg-gray-50 rounded-lg p-6">
                          <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                            <BarChart3 className="w-5 h-5 mr-2" /> Weather
                            Metrics
                          </h3>
                                ...
                                <div>
                                  ...
                                  <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div
                                      ...
                                      style={{
                                        width: `${Math.min(
                                          (weatherData.main.feels_like / 50) *
                                            100,
                                          100
                                        )}%`,
                                      }}
                        ...
                        <div className="bg-white rounded-lg p-6">
                          <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                            <Wind className="w-5 h-5 mr-2" /> Wind Analysis
                          </h3>
                              ...
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-red-700">
                                    Extreme Qatar heat (
                                    {Math.round(weatherData.main.temp)}°C)
                                    detected. Activate emergency cooling:
                                    increase drip irrigation to 3x frequency,
                                    deploy shade nets, and consider midday
                                    misting for greenhouse crops. Avoid field
                                    work during 10 AM - 4 PM.
                                  </p>
                                </div>
                              </div>
                            </div>
                              ...
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-yellow-700">
                                    ...
                                    Monitor plants for signs of stress and
                                    consider misting systems for greenhouse
                                    crops.
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-blue-700">
                                    Strong winds (
                                    {Math.round(weatherData.wind.speed * 3.6)}{" "}
                                    km/h) - possible Shamal conditions. Secure
                                    greenhouse panels, reinforce shade
                                    structures, protect date palm fronds, and
                                    postpone all spraying operations. Monitor
                                    sand drift around crops.
                                  </p>
                                </div>
                              </div>
                            </div>
                              ...
                              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                                <div className="flex items-center">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-green-700">
                                      Excellent conditions for Qatar desert
                                      farming! Perfect for early morning field
                                      operations, greenhouse maintenance, and
                                      precision irrigation adjustments. Ideal
                                      time for date palm care and vegetable
                                      transplanting.
                                    </p>
                                  </div>
                                </div>
                        ...
                        <div className="bg-indigo-50 rounded-lg p-6">
                          <h3 className="text-lg font-semibold text-indigo-800 mb-4 flex items-center">
                            <Target className="w-5 h-5 mr-2" /> Farm
                            Recommendations
                          </h3>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="bg-white rounded p-4">
                              <h4 className="font-medium text-gray-800 mb-2 flex items-center">
                                <Sprout className="w-4 h-4 mr-2" /> Crop Care
                              </h4>
                              <p className="text-sm text-gray-600">
